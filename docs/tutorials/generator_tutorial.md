...
* `resolution_ppdot_xray`: the resolution in bins or pixels of the maps containing the $P-\dot{P}$ information for the 
   neutron stars detected in X-ray.
* `generate_xray`: option specifying if the maps related to the X-ray surveys are produced (default is False).
* `filter_young_xdins`: option specifying if only young magnetars and XDINSs are considered in the X-ray population (default is False).
