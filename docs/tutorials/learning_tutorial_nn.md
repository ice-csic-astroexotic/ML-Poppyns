...
```
The correspondence between maps and indices is then the following:
...
`survey_PMPS_ppdot_map`, `survey_SMPS_ppdot_map`, `survey_HTRU_ppdot_map` (indices 12, 13, 14) and the labels `B_initial_log10_mean` and `P_initial_log10_mean` (indices 20, 21).
