...
In this way the multirun mode is used and a directory structure :code:`multirun/YYYY-MM-DD/hh-mm-ss/simulated_population_index` is created, where :code:`YYYY-MM-DD` and :code:`hh-mm-ss` folders names are the date and time when the simulation has been performed and the :code:`simulated_population_index` folders names are :code:`0, 1, 2, ...`. In each one of these folders an output file :code:`final_population.txt` and a folder named :code:`.hydra` are created. The file :code:`final_population.txt` contains all the physical information (like the final position, the final velocity...) of each star after the evolution. The folder :code:`.hydra` contains a file named :code:`config.yaml` storing the parameter values from where the population has been initialized.
...
In this way a population is simulated for each couple of value of :code:`vk_c` and :code:`h_c`.