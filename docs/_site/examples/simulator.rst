...
If you want to run simulations with a huge parameter sweep you can use the wrapper or helper script called :code:`simulation_helper` in the :code:`scripts` folder.
This script allows to choose the type of simulation you want to run (:code:`simulate_population_dyn`, :code:`simulate_population_magrot_det` or :code:`simulate_population_full`)  and to specify the simulation parameters with two types of sampling, determined by the argument :code:`--sampling_type`.

If :code:`--sampling_type = grid` you should provide the parameters in a linear spacing format :code:`--parameter [low] [high] [steps]`:

.. code-block:: bash

  python scripts/simulation_helper.py --simulation_type simulate_population_dyn --output_dir simulated_data --kick_model "km_exp" --vk_c 100.0 200.0 100 --sampling_type grid

This example will run the dynamical simulation only and generate a sweep of :code:`100` uniformly spaced samples for the :code:`vk_c` parameter in the range :code:`[100.0, 200.0]` using the :code:`km_exp` kick model and the results will be dumped in the specified :code:`simulated_data` folder.
...
.. code-block:: bash
  ...
  python scripts/simulation_helper.py --simulation_type simulate_population_dyn --output_dir simulated_data --kick_model "km_exp" --vk_c 100.0 200.0 --sampling_type random --sampling_size 100
...
* :code:`h_c` for the galactic height distribution model of birth places.
...
.. code-block:: bash
  ...
  python scripts/simulation_helper.py --simulation_type simulate_population_dyn --output_dir simulated_data --kick_model "km_exp" --vk_c 100.0 200.0 100 --h_c 0.01 2 10 --sampling_type grid
