...
1) Simulate the entire population all together, which means initializing the entire population formed by N neutron stars from some initial conditions, evolve it in time and finally apply the survey models to select the detected neutron stars.
   For this approach you have to run the script :code:`pypopsyn/simulator/simulate_population_full.py`.
...
2) Simulate first the dynamical evolution of a huge number of neutron stars (without specifying the birth rate) by using the script :code:`pypopsyn/simulator/simulate_population_dyn.py`.
   The output of the dynamical evolution can be then used as a database for running the other steps of the simulations.
   Through the script :code:`pypopsyn/simulator/simulate_population_magrot_det.py` you then select stars from the dynamically evolved database according to the sky coverage of a given survey, evolve their properties in time and finally establish if they are detected or not by the surveys.
...
If you want to run simulations with a large parameter sweep, you can use the helper script called :code:`simulation_helper` in the :code:`scripts` folder.
...
::
  ...
  python utilities/simulation_helper.py --simulation_type simulate_population_dyn --output_dir simulated_data --kick_model "km_exp" --vk_c 100.0 200.0 100 --sampling_type grid
...
::
  ...
  python utilities/simulation_helper.py --simulation_type simulate_population_dyn --output_dir simulated_data --kick_model "km_exp" --vk_c 100.0 200.0 --sampling_type random --sampling_size 100
...
* :code:`vk_c` for the exponential kick-velocity model :code:`km_exp`;
* :code:`sigma_k` for the Maxwell kick-velocity model :code:`km_maxwell`;
* :code:`h_c` for the Galactic scale-height-distribution model of birth places.

For the magneto-rotational evolution:

* :code:`P_initial_mean` and :code:`P_initial_sigma` for the birth spin-period :code:`normal` distribution model;
* :code:`P_initial_log10_mean` and :code:`P_initial_log10_sigma` for the birth spin-period :code:`log-normal` distribution model;
* :code:`B_initial_log10_mean` and :code:`B_initial_log10_sigma` for the birth magnetic-field lognormal distribution;
...
::
  ...
  python utilities/simulation_helper.py --simulation_type simulate_population_dyn --output_dir simulated_data --kick_model "km_exp" --vk_c 100.0 200.0 100 --h_c 0.01 2 10 --sampling_type grid
...
To visualize the outcome of a given simulation a number of jupyter notebooks are provided in :code:`tutorials/analysis_notebook`.
In particular to check the output of a full simulation run through the script :code:`pypopsyn/simulator/simulate_population_full.py` you can use the following ones:
...
To check the output of a simulation run through the script :code:`pypopsyn/simulator/simulate_population_magrot_det.py` and compare it to observations you can use the :code:`survey_sim_vs_obs.ipynb` notebook.