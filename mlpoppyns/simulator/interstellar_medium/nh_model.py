...
def compute_NH(RA: np.ndarray, DEC: np.ndarray, d: np.ndarray) -> np.ndarray:
    ...
    map_path = pathlib.Path().joinpath(
        ...
        "mlpoppyns/simulator/interstellar_medium/ebv_map.npz",
    )
    ...
    try:
        if not os.path.exists(map_path):
            ...
            log.info(
                ...
                f"Once the download is completed, the map will be saved in mlpoppyns/simulator/interstellar_medium/."
            )
        ...
        log.error(
            ...
            "mlpoppyns/simulator/config_simulator.py file."
        )
    ...
    nh = ebv * nh_conv_fac
    N_H = np.asarray(nh * 10**21, dtype=float)
