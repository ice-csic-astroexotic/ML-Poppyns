...
def calculate_xray_emission(
    ...
    S_x_bb_abs = np.zeros_like(L_x_therm)
    S_x_rcs_abs = np.zeros_like(L_x_therm)
    N_H = np.zeros_like(L_x_therm)

    (
        S_x_bb_abs[xray_bright_mask],
        S_x_rcs_abs[xray_bright_mask],
        N_H[xray_bright_mask],
    ) = flux_xray_absorbed(
        L_x_therm[xray_bright_mask],
        B[xray_bright_mask],
        ra[xray_bright_mask],
...
def outburst_filter_probabilistic(
    ...
    outburst_prob[age_mask_1] = np.random.uniform(
        0.40, 0.85, size=len(age[age_mask_1])
    )
    age_mask_2 = (age > 100) & (age <= 400)
    outburst_prob[age_mask_2] = np.random.uniform(
        0.1, 0.4, size=len(age[age_mask_2])
    )
    age_mask_3 = (age > 400) & (age <= 1000)
