...
def test_case_1():
    data = {
        "x": np.array([1.0e-1, 1.0e0]),
        "y": np.array([1.0e-1, 1.0e1]),
        "x_range": (1.0e-3, 10.0),
        ...
        "x_edges_expected": np.array([-3.0, -2.0, -1.0, 0.0, 1.0]),
        "y_edges_expected": np.array([-2.0, -1.0, 0.0, 1.0, 2.0]),
    }
...
def test_case_2():
    data = {
        ...
        "n_x_bins": 5,
        "n_y_bins": 5,
        "x_expected": np.array([3.0, 5.0]),
        "y_expected": np.array([10.0, 50.0]),
...
def test_case_3():
    data = {
        ...
        "b": np.array([5.0, np.nan, 7.0, 8.0]),
        "c": np.array([9.0, 10.0, 11.0, 12.0]),
        "a_clean": np.array([1.0, 4.0]),
        "b_clean": np.array([5.0, 8.0]),
        "c_clean": np.array([9.0, 12.0]),
    }
...
def test_check_range_above_zero_01():
    ...
    with pytest.raises(ValueError, match="Value range has to be above zero."):
        mu.check_range_above_zero(r_range)
...
def test_log_scale_vs_linear_scale_01(test_case_1):
    ...
    assert np.isclose(x, test_case_1["x_expected"]).all()
    assert np.isclose(y, test_case_1["y_expected"]).all()
...
def test_log_scale_vs_linear_scale_02(test_case_2):
    ...
    assert np.isclose(x, test_case_2["x_expected"]).all()
    assert np.isclose(y, test_case_2["y_expected"]).all()
...
def test_remove_nan_entries(test_case_3):
    """
    Verifying that nan entries are correctly removed.
    """

    a_clean, b_clean, c_clean = mu.remove_nan_entries(
        test_case_3["a"], test_case_3["b"], test_case_3["c"]
    )

    assert np.allclose(a_clean, test_case_3["a_clean"])
    assert np.allclose(b_clean, test_case_3["b_clean"])
    assert np.allclose(c_clean, test_case_3["c_clean"])
