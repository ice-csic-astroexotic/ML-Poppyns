"""
    P-flux map generation routines.
...
def generate_p_flux_map(
    ...
    p_resolution: int,
    flux_resolution: int,
    pflux_maps_dictionary: dict,
    p_limits: typing.Tuple[float, float] = (0.01, 100.0),
    flux_limits: typing.Tuple[float, float] = (1.0e-15, 1.0e-9),
) -> None:
    """
    This method generates a discrete P-flux map with great flexibility, the
    ...
    p_fluxes_map_generators[map_type](
        periods,
        p_limits,
        fluxes,
        flux_limits,
    ...
    pflux_maps_dictionary.setdefault("input:" + map_name, []).append(
        pflux_map_filename
    )

    log.info("{} generated...".format(pflux_map_filename))
