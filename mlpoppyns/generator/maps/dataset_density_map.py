"""
    Dataset density map generation.

    This module provides methods for creating density maps from point distributions as part of dataset generation.
    It selects the appropriate map generation routine based on the requested output representation (image or array)
    and density estimation method (histogram or KDE). It saves the resulting map to disk, and records its path in the
    dataset dictionary.
