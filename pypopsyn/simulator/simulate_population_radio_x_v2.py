...
def initialize_x_surveys(cfg: dict) -> Tuple[dict, RectBivariateSpline]:
    """
...
def apply_surveys_coverage(
    ...
    Args:
        ...
        age_cutoff (float): The maximum neutron star age to include in the survey coverage.
...
def radio_detection(
    ...
    update_dictionary_detected_PMPS = {
        ...
        "idx_det_SMPS": idx_radio[detected_radio_PMPS].tolist(),
    }
...
def x_detection(
    ...
    Args:
        ...
        L_x_threshold (float): The luminosity threshold for X-ray detection.
    ...
    v_ls = dict_final_pop["v_ls"]
    P = dict_final_pop["P_final"]
    B_initial = dict_final_pop["B_initial"]
    B = dict_final_pop["B_final"]
    chi = dict_final_pop["chi_final"]
    idx = dict_final_pop["idx"]
    ...
    age = age[coverage_x]
    l_gal = l_gal[coverage_x]
    b_gal = b_gal[coverage_x]
    ra = ra[coverage_x]
    dec = dec[coverage_x]
    dist = dist[coverage_x]
    pm_ra = pm_ra[coverage_x]
    pm_dec = pm_dec[coverage_x]
    v_ls = v_ls[coverage_x]
    P = P[coverage_x]
    B_initial = B_initial[coverage_x]
    B = B[coverage_x]
    chi = chi[coverage_x]
    idx_x = idx[coverage_x]
    ...
    update_dictionary_detected_x = {
        ...
        "Pdot": P_dot[detected_x].tolist(),
        "L_x_therm": L_x_therm[detected_x].tolist(),
        "S_x_abs": S_x_abs[detected_x].tolist(),
        "idx_det_x": idx_x[detected_x].tolist(),
    }

    return update_dictionary_detected_x
...
def create_output_dataframe(
    ...
    Args:
        ...
        dictionary_detected_x (dict): Dictionary containing detected X-ray neutron star properties.

    Returns:
        Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]: DataFrames for
            detected neutron stars from PMPS, SMPS, HTRU low/mid, HTRU high, and X-ray surveys.
    """
    ...
    del dictionary_detected_x["idx_det_x"]
    ...
    return df_PMPS, df_SMPS, df_HTRU_low_mid, df_HTRU_high, df_x
...
def simulate_population(args) -> None:
    ...
    dictionary_detected_x, L_x_interpolator = initialize_x_surveys(cfg)
    ...
    with timewith.TimeWith(
        ...
        while (
            ...
            with timewith.TimeWith(
                ...
                update_dictionary_detected_x = x_detection(
                    pop_final, L_x_interpolator, L_x_threshold=1.0e30
                )

                n_detected_sim_x += len(update_dictionary_detected_x["age"])
                log.info(
                ...
                idx_det_x = update_dictionary_detected_x["idx_det_x"]
