...
def train(config: configuration_parser.ConfigurationParser) -> None:
    ...
    with timewith.TimeWith(
        ...
        with timewith.TimeWith(
            ...
            with open(
                f"{config.save_dir}/trained_model.pickle", "wb"
            ) as output_file:
                pickle.dump(density_estimator.cpu(), output_file)

            training_statistics_path = (
                f"{config.log_dir}/training_statistics.json"
            )
            ...
            plt.legend(
                bbox_to_anchor=(1.05, 1), frameon=False, loc=0, fontsize=10
            )

            f.savefig(
                f"{config.log_dir}/training_stats.pdf", bbox_inches="tight"
            )
